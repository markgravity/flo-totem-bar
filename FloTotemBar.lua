...
function FloTotemBar_OnLoad(self)
    ...
    if FLO_CLASS_NAME == "SHAMAN" and WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC then
...
function FloTotemBar_OnEvent(self, event, arg1, ...)
    ...
    elseif (event == "ADDON_LOADED" and arg1 == "FloTotemBar") or event == "UPDATE_BINDINGS" then
        if event == "ADDON_LOADED" then
            ...
            if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then
                hooksecurefunc("SetSpecialization", function()
                    changingSpec = true;
                end);
            end
        end
...
function FloTotemBar_CheckTrapLife(self, spellIdx, timestamp, event, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellId, spellName, ...)
    ...
    if spellName == "Effet Piège immolation" then
        spellName = "Effet Piège d'immolation"
    end
...
function FloTotemBar_SetupSpell(self, spell, pos)
    ...
    self.spells[pos] = { id = spell.id, name = spellName, addName = spell.addName, duration = duration, algo = algo, talented = spell.talented, talentedName = spell.talentedName, buffId = spell.buffId, isBuff = spell.isBuff };
...
function FloTotemBar_UpdatePosition(self)
    ...
    elseif WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC and FLO_CLASS_NAME == "SHAMAN" then
